...
# dataset related
flags.DEFINE_string('training_data_path', 'input/train_imputed_scaled_validation_filtered.csv', 'path to training data')
flags.DEFINE_string('testing_data_path', 'input/test_imputed_scaled_validation_filtered.csv', 'path to testing data')
...
flags.DEFINE_boolean('foml', True, 'use FOML instead of Reptile_validation_data')
...
def main():
    ...
    with open('input/y_test_crd.pkl', 'rb') as f:
        crd_column = pickle.load(f)
    X_train, y_train, X_test, y_test = generate_train_test(y_test_given=crd_column)
