...
def get_results_dementia(trained_models_dir, outfile_name, output_folder, sort_by, topn=3,):
    ...
    metrics = ['model', 'type', 'f2', 'gmean', 'bss', 'pr_auc', 'sensitivity', 'specificity',
               'accuracy', 'precision', 'recall', 'f1', 'auc']
        ...
        for root, dirs, files in os.walk(trained_models_dir):
            if 'testing_error_metrics.p' in files:
                ...
                with open(os.path.join(root, 'training_error_metrics.p'), 'rb') as f:
                    training_error_metrics = pickle.load(f)
                ...
                # add training error metrics
                classification_results = classification_results.append({
                    'model': model_count,
                    'type': 'training',
                    'gmean': '{}'.format(training_error_metrics['gmean']),
                    'f2': '{}'.format(training_error_metrics['f2']),
                    'bss': '{}'.format(training_error_metrics['bss']),
                    'pr_auc': '{}'.format(training_error_metrics['pr_auc']),
                    'sensitivity': '{}'.format(training_error_metrics['sensitivity']),
                    'specificity': '{}'.format(training_error_metrics['specificity']),
                    # 'ppv': '{}'.format(np.mean(risk_df_high['risk_scores'])),
                    'accuracy': '{}'.format(training_error_metrics['accuracy']),
                    'precision': '{}'.format(training_error_metrics['precision']),
                    'recall': '{}'.format(training_error_metrics['recall']),
                    'f1': '{}'.format(training_error_metrics['f1']),
                    'auc': '{}'.format(training_error_metrics['roc']),
                    # 'bss', 'pr_auc', 'sensitivity', 'specificity'
                }, ignore_index=True)


                # add an empty row
                classification_results = classification_results.append(pd.Series(), ignore_index=True)
