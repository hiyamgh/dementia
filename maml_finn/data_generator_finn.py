...
class DataGenerator(object):
    ...
    def make_data_tensor(self, train=True):
        ...
        for _ in range(num_total_batches):
            sampled_character_folders = random.sample(folders, self.num_classes) # sampling 5 character folders (each folder is a separate class)
            random.shuffle(sampled_character_folders) # num_samples_per_class is 2, so from each folder (class), get two images, assign them the same label
