...
class AdvancedEvaluator:
    ...
    def __init__(self, df, df_train, df_test, target_variable,
        ...
        #  scaling mechanism
        if scaling not in ['minmax', 'z-score', 'robust', None]:
        ...
        if fp_file is not None:
            ...
            self.identify_frequent_patterns()

        self.classification_results = pd.DataFrame(columns=['model', 'accuracy', 'precision', 'recall',
                                                            'f1', 'auc'], index=False)
    ...
    def classify(self, trained_model, trained_model_name, nb_bins=10):
        ...
        self.evaluate_predictions(y_test=y_test, y_pred=y_pred, model_name=trained_model_name)
    ...
    def save_results(self):
        self.mkdir(self.plots_output_folder)
        self.classification_results.to_csv('error_metrics_shallow.csv', index=False)
    ...
    def evaluate_predictions(self, y_test, y_pred, model_name):
        ...
        self.classification_results = self.classification_results.append({
            'model': model_name,
    ...
    def identify_frequent_patterns(self):
        ...
        self.freqItemSet, self.rules = fpgrowth(itemSetList, minSupRatio=0.8, minConf=0.8)
        fps = ['fp{}'.format(i) for i in range(1, len(self.freqItemSet) + 1)]
    ...
    def fp_in_df_raw(self, fps, df):
        col_types = df.dtypes
        indices_who_has_fp = []
        for index, row in df.iterrows():
            add_index = True
            for curr_fp in self.fp_dict[fps]:
