...
if __name__ == '__main__':
    ...
    for topn in [10, 20]:
        ...
        for opt in optimizations:
            ...
            for enc in encodings:
                ...
                out_proba = '../output/output_probabilistic/top_{}/{}/{}/'.format(topn, opt, enc)
                mkdir(out_proba)

                out_regular = '../output/output_regular/top_{}/{}/{}/'.format(topn, opt, enc)
                mkdir(out_regular)
                ...
                for model, model_name, proba, one_class in [
                    ...
                    if proba:
                        ...
                        # In regular non probabilistic models, we will optmize only for f2
                        if opt == 'f2':
                            print('\nModel: {}, proba: {}, one_class: {}'.format(model_name, proba, one_class))
    ...
    for topn in [10, 20]:
        ...
        # results[name] = {}
        for enc in encodings:
            out_one_class = '../output/output_one_class/top_{}/{}/'.format(topn, enc)
            mkdir(out_one_class)
