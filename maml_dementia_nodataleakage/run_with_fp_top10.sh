                  ...
                  for wt in ${weights[@]}; do
                    for ss in ${sampling_strategy[@]}; do
                       for e in ${encoding[@]}; do
                         ...
                         echo "pretrain_iterations: 0 metatrain_iterations: ${miter} meta_batch_size: ${mbs} meta_lr: ${mlr} update_batch_size: ${mbs} update_lr: ${ulr} num_updates: ${nu} dim_hidden: ${dh} model_num: $SLURM_ARRAY_TASK_ID activation_fn: ${afn}  includefp: ${ifp} fp_file: dementia_colsmeta_top10/fps_dementia_${fp_supp}.pickle colsmeta_file dementia_colsmeta_top10/colsmeta_dementia_${fp_supp}.pickle weights_vector ${wt} --sampling_strategy ${ss} --top_features 10 --logdir dementia_with_fp_top10 --categorical_encoding ${e}"
                         python main.py  --pretrain_iterations 0 --metatrain_iterations ${miter} --meta_batch_size ${mbs} --meta_lr ${mlr} --update_batch_size ${mbs} --update_lr ${ulr} --num_updates ${nu} --dim_hidden ${dh} --model_num $SLURM_ARRAY_TASK_ID --activation_fn ${afn} --categorical_encoding ${e} --include_fp ${ifp}  --fp_file "dementia_colsmeta_top10/fps_dementia_${fp_supp}.pickle" --colsmeta_file "dementia_colsmeta_top10/colsmeta_dementia_${fp_supp}.pickle" --weights_vector ${wt} --sampling_strategy ${ss} --top_features 10 --logdir "dementia_with_fp_top10"
                         fi
