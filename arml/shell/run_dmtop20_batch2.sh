                  ...
                  for ss in ${sampling_strategy[@]}; do
                    for e in ${encoding[@]}; do
                        if [ $USCOUNTER -eq $SLURM_ARRAY_TASK_ID ]; then
                          ...
                          echo "metatrain_iterations: ${miter} meta_batch_size: ${mbs} meta_lr: ${mlr} update_batch_size: ${mbs} update_lr: ${ulr} num_updates: ${nu} dim_hidden: ${dh} model_num: $SLURM_ARRAY_TASK_ID activation_fn: ${afn} num_vertex ${nv} weights_vector ${wt} --sampling_strategy ${ss} --top_features 20 --logdir dm_top20/ --categorical_encoding ${e}"
