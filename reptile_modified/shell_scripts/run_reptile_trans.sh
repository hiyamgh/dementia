                                                        ...
                                                        for e in ${encoding[@]}; do
                                                            for tf in ${top_features[@]}; do
                                                                if [ $USCOUNTER -eq $SLURM_ARRAY_TASK_ID ]; then
                                                                    ...
                                                                    python main_reptile_trans.py --shots ${shts} --inner_batch ${ib} --inner_iters ${ii} --learning_rate ${lr} --meta_step ${ms} --meta_step_final ${msf} --meta_batch ${mb} --meta_iters ${mi} --eval_batch ${eb} --eval_iters ${ei} --dim_hidden ${dh} --activation_fn ${af} --weights_vector ${w} --sampling_strategy ${ss} --categorical_encoding ${e} --top_features ${tf} --logdir "reptile_trans_trained_models/${tf}/"
                                                                fi
